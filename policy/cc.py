...
from transport.utils import uint32_add, uint32_gt
from scipy import stats

BURST_DELTA_THRESHOLD_MS = 5
BURST_MAX_DURATION = 100
...
@dataclass
class TimestampGroupMillisecond:
    def __init__(self, send_time: int, arrival_time: int) -> None:
        self.arrival_time: Optional[int] = None
...
@dataclass
class InterArrivalDeltaMillisecond:
    def __init__(self, send_time_delta: int, arrival_time_delta: int, size_delta: int) -> None:
        self.send_time_delta = send_time_delta
        self.arrival_time_delta = arrival_time_delta
...
class InterArrivalMillisecond:
    """
    Inter-arrival time and size filter.

    Adapted from the webrtc.org codebase modules/congestion_controller/goog_cc/inter_arrival_delta.h.
    """

    def __init__(self, group_length: int) -> None:
        self.group_length = group_length
        self.current_group: Optional[TimestampGroupMillisecond] = None
        self.previous_group: Optional[TimestampGroupMillisecond] = None

    def compute_deltas(
        self, send_time: int, arrival_time: int, packet_size: int
    ) -> Optional[InterArrivalDeltaMillisecond]:
        deltas = None
        if self.current_group is None:
            self.current_group = TimestampGroupMillisecond(send_time, arrival_time)
        elif self.current_group.first_send_time > send_time:
            return deltas
        elif self.new_timestamp_group(send_time, arrival_time):
            if self.previous_group is not None:
                deltas = InterArrivalDeltaMillisecond(
                    send_time_delta=self.current_group.last_send_time - self.previous_group.last_send_time,
            ...
            self.previous_group = self.current_group
            self.current_group = TimestampGroupMillisecond(send_time, arrival_time)
        elif send_time > self.current_group.last_send_time:
            self.current_group.last_send_time = send_time

        self.current_group.size += packet_size
        self.current_group.arrival_time = arrival_time
        return deltas

    def belongs_to_burst(self, send_time: int, arrival_time: int) -> bool:
        send_time_delta = send_time - self.current_group.last_send_time
        arrival_time_delta = arrival_time - self.current_group.arrival_time
        return arrival_time - self.current_group.first_arrival_time < BURST_MAX_DURATION \
            and arrival_time_delta <= BURST_DELTA_THRESHOLD_MS \
            and arrival_time_delta < send_time_delta

    def new_timestamp_group(self, send_time: int, arrival_time: int) -> bool:
        if self.belongs_to_burst(send_time, arrival_time):
            return False
        else:
            send_time_delta = send_time - self.current_group.first_send_time
            return send_time_delta > self.group_length
...
class TrendlineEstimator:
    ...
    @dataclass
    class PacketTiming:
        def __init__(self, arrival_time_ms: int, smoothed_delay_ms: int, raw_delay_ms: int):
            self.arrival_time_ms = arrival_time_ms
            self.smoothed_delay_ms = smoothed_delay_ms
            self.raw_delay_ms = raw_delay_ms
    ...
    def __init__(self):
        ...
        self.last_threshold_update_ms = None
    ...
    def update(self, recv_delta_ms: int, send_delta_ms: int,
        ...
        if self.first_arrival_time_ms is None:
            self.first_arrival_time_ms = arrive_time_ms

        self.accumulated_delay += delta_ms
        self.smoothed_delay = self.SMOOTHING_COEFF * self.smoothed_delay + \
            (1 - self.SMOOTHING_COEFF) * self.accumulated_delay

        self.delay_hist_.append(TrendlineEstimator.PacketTiming(
            arrival_time_ms=arrive_time_ms - self.first_arrival_time_ms,
            smoothed_delay_ms=self.smoothed_delay,
            raw_delay_ms=self.accumulated_delay
        ))
        while len(self.delay_hist_) > self.WINDOW_SIZE:
            self.delay_hist_.popleft()
    ...
    def detect(self, trend: float, send_delta_ms: int, arrival_time_ms: int):
        ...
        if modified_trend > self.threshold:
            if self.time_over_using is None:
                ...
                self.time_over_using += send_delta_ms
            self.overuse_count += 1
            if (self.time_over_using > TrendlineEstimator.OverUsingTimeThreshold and
                    self.overuse_count > 1):
                if trend > self.prev_trend:
                    self.time_over_using = None
                    self.overuse_count = 0
                    self.usage = BandwidthUsage.OVERUSING

        elif modified_trend < -self.threshold:
            self.time_over_using = None
            self.overuse_count = 0
            self.usage = BandwidthUsage.UNDERUSING
        else:
            self.time_over_using = None
            self.overuse_count = 0
            self.usage = BandwidthUsage.NORMAL
        self.prev_trend = trend
        self.update_threshold(modified_trend, now_ms=arrival_time_ms)

    def update_threshold(self, modified_trend: float, now_ms: int):
        if self.last_threshold_update_ms is None:
            self.last_threshold_update_ms = now_ms

        if abs(modified_trend) > self.threshold + MAX_ADAPT_OFFSET_MS:
            self.last_threshold_update_ms = now_ms
            return

        k = 0.0
        if abs(modified_trend) < self.threshold:
            k = self.k_down
        else:
            k = self.k_up

        time_delta_ms = min(now_ms - self.last_threshold_update_ms, 100)
        self.threshold += k * (abs(modified_trend) - self.threshold) * time_delta_ms
        self.threshold = min(self.threshold, 600.0)
        self.threshold = max(self.threshold, 6.0)
        self.last_threshold_update_ms = now_ms
...
class SendSideDelayBasedBitrateEstimator:
    def __init__(self) -> None:
        ...
        self.rate_control = AimdRateControl()
        self.last_update_ms: Optional[int] = None
        self.last_arrival_time_ms: Optional[int] = None

    def add(
        self, arrival_time_ms: int, send_time_ms: int, payload_size: int
    ) -> Optional[int]:
        if self.last_arrival_time_ms is None:
        ...
        if deltas is not None:
            ...
            self.estimator.update(
                ...
                arrive_time_ms=arrival_time_ms,
            )
        ...
        if not update_estimate:
            if (
                ...
                update_estimate = True
            elif self.estimator.state() == BandwidthUsage.OVERUSING:
                update_estimate = True
